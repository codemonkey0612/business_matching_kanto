  ...
  <div class="bg-white border border-[#E8E4DC] rounded p-5 space-y-4"
    ...
    <div>
      ...
      <input type="text" id="company_name" name="company_name"
             value="{{ old('company_name', $participant->company?->company_name) }}"
             required placeholder="株式会社〇〇"
             class="form-input @error('company_name') is-error @enderror">
      @error('company_name')
    ...
    <div class="grid grid-cols-2 gap-3">
      ...
      <div>
        ...
        <input type="text" id="name_kana" name="name_kana"
               value="{{ old('name_kana', $participant->name_kana) }}"
    ...
    <div class="grid grid-cols-2 gap-3">
      <div>
        ...
        <input type="text" id="role_title" name="role_title"
               value="{{ old('role_title', $participant->role_title) }}"
    ...
    <div>
      ...
      <select id="industry_master_id" name="industry_master_id" required class="form-input bg-white">
        ...
        @foreach($industries as $ind)
          <option value="{{ $ind->id }}"
            {{ old('industry_master_id', $participant->company?->industry_master_id) == $ind->id ? 'selected' : '' }}>
            {{ $ind->name }}
          </option>
        @endforeach
      </select>
    ...
    <div class="grid grid-cols-3 gap-3">
      <div class="col-span-2">
        ...
        <input type="text" id="address_text" name="address_text"
               value="{{ old('address_text', $participant->company?->address_text) }}"
        ...
        <select id="prefecture_master_id" name="prefecture_master_id" required class="form-input bg-white">
          ...
          @foreach($prefectures as $pref)
            <option value="{{ $pref->id }}"
              {{ old('prefecture_master_id', $participant->company?->prefecture_master_id) == $pref->id ? 'selected' : '' }}>
              {{ $pref->name }}
            </option>
          @endforeach
        </select>
  ...
  <div class="bg-white border border-[#E8E4DC] rounded p-5 space-y-4"
    ...
    <div>
      ...
      <textarea id="business_summary_1" name="business_summary_1" rows="3" required
                ...
                style="height:auto">{{ old('business_summary_1', $participant->profile?->business_summary_1) }}</textarea>
    </div>
    <div>
      ...
      <textarea id="business_summary_2" name="business_summary_2" rows="3"
                ...
                class="form-input resize-none"
                style="height:auto">{{ old('business_summary_2', $participant->profile?->business_summary_2) }}</textarea>
    </div>
  </div>
</section>

{{-- ===== 03. 希望目的 ===== --}}
@php $selectedPurposes = old('purpose_ids', $participant->purposes->pluck('id')->toArray()); @endphp
<section class="mb-6 reveal" style="transition-delay:0.16s">
  ...
  <div class="bg-white border border-[#E8E4DC] rounded p-5"
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <label class="cursor-pointer" for="purpose_{{ $p->id }}">
        <input type="checkbox" id="purpose_{{ $p->id }}" name="purpose_ids[]"
               value="{{ $p->id }}" class="sr-only peer"
               {{ in_array($p->id, $selectedPurposes) ? 'checked' : '' }}>
    ...
    @error('purpose_ids')
      <p class="mt-3 text-enji" style="font-size:13px">{{ $message }}</p>
    @enderror
  </div>
</section>
...
<section class="mb-6 reveal" style="transition-delay:0.24s">
  ...
  {{-- Alpine.jsで折りたたみ・検索・カウントを管理 --}}
  <div class="bg-white border border-[#E8E4DC] rounded"
       style="box-shadow: 0 1px 2px rgba(0,0,0,0.04)"
       id="issue-section"
       x-data="{
         count: {{ count($selectedIssues) }},
         search: '',
         cats: {
           @foreach($issuesByCategory as $cat => $catIssues)
             ...
             @endphp
             '{{ addslashes($cat) }}': {{ $hasSelected ? 'true' : 'false' }},
           @endforeach
         },
    ...
    <div class="border-b border-[#F0EDE6] last:border-b-0">
      ...
      <button type="button"
              ...
              :aria-expanded="cats['{{ $catKey }}'].toString()">
        <span class="font-medium text-sumi" style="font-size:14px">{{ $cat }}</span>
        <div class="flex items-center gap-2 shrink-0">
          @if($catSelected > 0)
            <span class="inline-block px-2 py-0.5 rounded-full bg-[#F8ECEE] text-enji border border-[#E8C9CE]"
                  style="font-size:11px">{{ $catSelected }}件</span>
          @endif
          <svg class="w-4 h-4 text-fumi transition-transform ease-out"
               :class="cats['{{ $catKey }}'] ? 'rotate-180' : ''"
      ...
      <div x-show="showCat('{{ $catKey }}')"
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <label x-show="showItem('{{ addslashes($issue->name) }}')"
                 class="cursor-pointer" for="issue_{{ $issue->id }}">
            <input type="checkbox" id="issue_{{ $issue->id }}" name="issue_ids[]"
                   value="{{ $issue->id }}" class="sr-only peer"
    ...
    <div class="px-5 py-4 bg-washi rounded-b">
      ...
      <input type="text" id="issue_other_text" name="issue_other_text"
             value="{{ old('issue_other_text', $participant->profile?->issue_other_text) }}"
...
<section class="mb-8 reveal" style="transition-delay:0.10s">
  ...
  <div class="bg-white border border-[#E8E4DC] rounded"
       ...
       id="partner-section"
       x-data="{
         count: {{ count($selectedPartners) }},
         search: '',
         cats: {
           @foreach($partnersByCategory as $cat => $catPartners)
             @php
    ...
    <div class="border-b border-[#F0EDE6] last:border-b-0">
      ...
      <div x-show="showCat('{{ $catKey }}')"
           ...
           class="px-5 pb-4 pt-1">
        <div class="flex flex-wrap gap-2">
          @foreach($catPartners as $pt)
          <label x-show="showItem('{{ addslashes($pt->name) }}')"
                 class="cursor-pointer" for="partner_{{ $pt->id }}">
            <input type="checkbox" id="partner_{{ $pt->id }}" name="partner_type_ids[]"
                   value="{{ $pt->id }}" class="sr-only peer"
                   {{ in_array($pt->id, $selectedPartners) ? 'checked' : '' }}>
            <span class="inline-flex items-center px-3 py-1.5 rounded-full font-medium select-none
                         transition-colors ease-out
                         border border-kiwari bg-white text-fumi
    ...
    <div class="px-5 py-4 bg-washi rounded-b">
      ...
      <input type="text" id="partner_other_text" name="partner_other_text"
             value="{{ old('partner_other_text', $participant->profile?->partner_other_text) }}"
             placeholder="上記にない相手を探している場合は入力してください"
             class="form-input">
    </div>
  </div>

  @error('partner_type_ids')
    <p class="mt-2 text-enji" style="font-size:13px">{{ $message }}</p>
  @enderror
</section>
