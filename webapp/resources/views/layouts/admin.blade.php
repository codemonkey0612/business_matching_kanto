...
<title>@yield('title', '管理画面') | BizMatch</title>
...
<header class="m-topbar">
  <div style="display:flex;align-items:center;gap:10px">
    <div style="width:34px;height:34px;background:linear-gradient(145deg,#4F46E5 0%,#7C3AED 100%);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 3px 10px rgba(99,102,241,0.40),inset 0 1px 0 rgba(255,255,255,0.18)">
      <svg width="20" height="20" viewBox="0 0 32 32" fill="none" aria-hidden="true">
        <circle cx="16" cy="16" r="12" fill="white" opacity="0.08"/>
        <polygon points="0,6 12,16 0,26" fill="white" opacity="0.84"/>
        <polygon points="32,6 20,16 32,26" fill="white" opacity="0.84"/>
        <circle cx="16" cy="16" r="6" fill="white"/>
        <circle cx="16" cy="16" r="3" fill="none" stroke="rgba(99,102,241,0.25)" stroke-width="1.5"/>
      </svg>
    </div>
    <div>
      <div style="font-size:15px;font-weight:800;color:#1E293B;letter-spacing:-0.025em;line-height:1;font-family:'Inter',sans-serif">BizMatch</div>
      <div style="font-size:9px;font-weight:600;color:#94A3B8;letter-spacing:0.10em;text-transform:uppercase;font-family:'Inter',sans-serif;margin-top:3px">Admin Console</div>
    </div>
  </div>
</header>
...
<aside class="sidebar">
  {{-- Logo --}}
  <div style="padding:22px 18px 18px;border-bottom:1px solid #F1F5F9">
    <div style="display:flex;align-items:center;gap:11px">
      <div style="width:40px;height:40px;background:linear-gradient(145deg,#4F46E5 0%,#7C3AED 100%);border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 4px 16px rgba(99,102,241,0.42),inset 0 1px 0 rgba(255,255,255,0.20)">
        <svg width="23" height="23" viewBox="0 0 32 32" fill="none" aria-hidden="true">
          <circle cx="16" cy="16" r="12" fill="white" opacity="0.09"/>
          <polygon points="0,6 12,16 0,26" fill="white" opacity="0.84"/>
          <polygon points="32,6 20,16 32,26" fill="white" opacity="0.84"/>
          <circle cx="16" cy="16" r="6" fill="white"/>
          <circle cx="16" cy="16" r="3" fill="none" stroke="rgba(99,102,241,0.28)" stroke-width="1.5"/>
        </svg>
      </div>
      <div>
        <p style="font-size:15.5px;font-weight:800;color:#1E293B;letter-spacing:-0.025em;line-height:1;font-family:'Inter',sans-serif">BizMatch</p>
        <p style="font-size:9.5px;color:#94A3B8;margin-top:4px;letter-spacing:0.11em;text-transform:uppercase;font-family:'Inter',sans-serif">Admin Console</p>
      </div>
    </div>
  </div>
  ...
  <nav style="padding:14px 10px;flex:1" aria-label="管理メニュー">
    ...
    <a href="{{ route('admin.participants.index') }}"
       class="nav-item {{ request()->routeIs('admin.participants.*') ? 'active' : '' }}" style="margin-top:3px">
    ...
    <a href="{{ route('admin.notifications.index') }}"
       class="nav-item {{ request()->routeIs('admin.notifications.*') ? 'active' : '' }}" style="margin-top:3px">
...
<nav class="m-bottomnav" aria-label="メインメニュー">
  <a href="{{ route('admin.dashboard') }}"
     class="m-tab {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="{{ request()->routeIs('admin.dashboard') ? '2.2' : '1.6' }}" stroke-linecap="round" stroke-linejoin="round">
  ...
  <a href="{{ route('admin.participants.index') }}"
     class="m-tab {{ request()->routeIs('admin.participants.*') ? 'active' : '' }}">
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="{{ request()->routeIs('admin.participants.*') ? '2.2' : '1.6' }}" stroke-linecap="round" stroke-linejoin="round">
  ...
  <a href="{{ route('admin.notifications.index') }}"
     class="m-tab {{ request()->routeIs('admin.notifications.*') ? 'active' : '' }}">
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="{{ request()->routeIs('admin.notifications.*') ? '2.2' : '1.6' }}" stroke-linecap="round" stroke-linejoin="round">
