...
<div style="display:flex;flex-wrap:wrap;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:20px">
  ...
  <div style="display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,#0F172A,#1E293B);border-radius:14px;padding:11px 20px;flex-shrink:0;box-shadow:0 4px 16px rgba(0,0,0,0.18)">
    ...
    <div>
      <p style="font-size:22px;font-weight:800;color:#fff;line-height:1;letter-spacing:-0.03em;font-family:'Inter',sans-serif">{{ $logs->total() }}</p>
...
<form method="GET"
  ...
  <div style="position:relative">
    <select id="status-filter" name="status" class="f-select">
      ...
      <option value="sent"    {{ request('status') === 'sent'    ? 'selected' : '' }}>✓ 送信済み</option>
  ...
  <div style="margin-left:auto;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
    <span style="display:inline-flex;align-items:center;gap:5px;font-size:11.5px;color:#15803D;font-weight:600;background:#DCFCE7;padding:4px 10px;border-radius:100px;border:1px solid #A7F3D0;font-family:'Inter',sans-serif">
      <span style="width:5px;height:5px;border-radius:50%;background:currentColor"></span>{{ $sentCount }}
    </span>
    @if($failedCount > 0)
      <span style="display:inline-flex;align-items:center;gap:5px;font-size:11.5px;color:#DC2626;font-weight:600;background:#FEE2E2;padding:4px 10px;border-radius:100px;border:1px solid #FCA5A5;font-family:'Inter',sans-serif">
        <span class="dot-pulse" style="width:5px;height:5px;border-radius:50%;background:currentColor"></span>失敗 {{ $failedCount }}
      </span>
    @endif
    @if($pendingCount > 0)
      <span style="display:inline-flex;align-items:center;gap:5px;font-size:11.5px;color:#B45309;font-weight:600;background:#FEF3C7;padding:4px 10px;border-radius:100px;border:1px solid #FCD34D;font-family:'Inter',sans-serif">
        <span style="width:5px;height:5px;border-radius:50%;background:currentColor"></span>保留 {{ $pendingCount }}
      </span>
    @endif
  </div>
</form>
...
<div style="background:#fff;border:1px solid #E8EAED;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.07)">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:13px 20px;border-bottom:1px solid #F1F5F9;background:#FAFBFF">
    <p style="font-size:12.5px;color:#64748B;font-family:'Inter',sans-serif;font-weight:500">
      @if($logs->total() > 0)
        <span style="color:#1E293B;font-weight:700">{{ $logs->firstItem() }}–{{ $logs->lastItem() }}</span>
        <span style="color:#94A3B8"> / {{ $logs->total() }}件</span>
      @else
      ...
      <tbody>
        ...
        <tr class="{{ $rowClass }}">
          <td style="padding-left:20px;width:44px">
            <span style="font-size:11px;font-weight:600;color:#CBD5E1;font-family:'Inter',sans-serif">{{ str_pad($loop->index + ($logs->currentPage()-1)*$logs->perPage() + 1, 2, '0', STR_PAD_LEFT) }}</span>
          </td>
          <td>
            <div style="display:flex;align-items:center;gap:10px">
              <div style="width:36px;height:36px;border-radius:10px;background:{{ $avc }};display:flex;align-items:center;justify-content:center;color:white;font-size:14px;font-weight:800;flex-shrink:0;font-family:'Inter',sans-serif;box-shadow:0 0 0 2px #fff,0 0 0 3.5px {{ $avc }}50">{{ $ini }}</div>
              <div>
                <p style="font-size:13px;font-weight:700;color:#1E293B;line-height:1.3;font-family:'Noto Sans JP',sans-serif;white-space:nowrap">{{ $log->target?->name ?? '—' }}</p>
                <p style="font-size:11px;color:#94A3B8;line-height:1.3;font-family:'Inter',sans-serif;white-space:nowrap;margin-top:1px">{{ $log->target_email }}</p>
              </div>
            </div>
          </td>
          <td style="max-width:220px">
            <p style="font-size:13px;color:#475569;font-family:'Noto Sans JP',sans-serif;line-height:1.5;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">{{ $log->mail_subject }}</p>
          </td>
          ...
          <td style="text-align:center">
            @if($log->retry_count > 0)
              <div style="display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:7px;background:#FEE2E2;border:1px solid #FCA5A5">
                <span style="font-size:12px;font-weight:800;color:#DC2626;font-family:'Inter',sans-serif">{{ $log->retry_count }}</span>
              </div>
            @else
          ...
          <td>
            @if($log->sent_at)
              <p style="font-size:13px;font-weight:600;color:#334155;font-family:'Inter',sans-serif;white-space:nowrap">{{ $log->sent_at->format('m/d') }}</p>
              <p style="font-size:11.5px;color:#94A3B8;font-family:'Inter',sans-serif;white-space:nowrap;margin-top:1px">{{ $log->sent_at->format('H:i') }}</p>
            @else
              <span style="font-size:12px;color:#CBD5E1;font-family:'Inter',sans-serif">—</span>
            @endif
          </td>
          <td>
            @php $fromName = $log->contactRequest?->fromParticipant?->name ?? '—'; @endphp
            @if($fromName !== '—')
              <div style="display:flex;align-items:center;gap:7px">
                ...
                <span style="font-size:13px;color:#475569;font-family:'Noto Sans JP',sans-serif;white-space:nowrap;font-weight:500">{{ $fromName }}</span>
              </div>
            @else
              <span style="font-size:12px;color:#CBD5E1;font-family:'Inter',sans-serif">—</span>
            @endif
