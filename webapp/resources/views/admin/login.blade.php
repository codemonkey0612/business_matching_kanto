  ...
  <div style="position:relative;z-index:10;flex:1;display:flex;flex-direction:column;padding:48px 52px">
    ...
    <div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding:40px 0">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:32px">
        @foreach(['参加者管理','統計ダッシュボード','通知追跡'] as $f)
    ...
    <p style="font-size:11.5px;color:rgba(255,255,255,0.30);font-family:'Inter',sans-serif">© {{ date('Y') }} BizMatch. All rights reserved.</p>
  </div>
</div>
      ...
      <form method="POST" action="{{ route('admin.login') }}" novalidate>
        ...
        <div style="margin-bottom:18px">
          ...
          <input type="text" id="login_id" name="login_id"
                 ...
                 class="field {{ $errors->has('login_id') ? 'err' : '' }}">
          @error('login_id')
            <p style="font-size:12px;color:#EF4444;margin-top:5px;font-family:'Noto Sans JP',sans-serif">{{ $message }}</p>
          @enderror
        </div>
