...
<div style="display:flex;flex-wrap:wrap;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:20px">
  ...
  <div style="display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,#1E293B,#334155);border-radius:14px;padding:11px 20px;flex-shrink:0;box-shadow:0 4px 16px rgba(0,0,0,0.15)">
    ...
    <div>
      <p style="font-size:22px;font-weight:800;color:#fff;line-height:1;letter-spacing:-0.03em;font-family:'Inter',sans-serif">{{ $participants->total() }}</p>
...
<form method="GET"
  ...
  <div style="position:relative;flex:1;min-width:200px">
    ...
    <input type="text" name="q" value="{{ request('q') }}" placeholder="氏名・メール・会社名で検索"
  ...
  <div style="position:relative;min-width:148px">
    <select name="status" class="s-select" aria-label="登録状態">
      ...
      <option value="completed" {{ request('status')==='completed'?'selected':'' }}>登録完了</option>
      <option value="draft"     {{ request('status')==='draft'?'selected':'' }}>登録途中</option>
    </select>
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:13px 20px;border-bottom:1px solid #F1F5F9;background:#FAFBFF">
    <p style="font-size:12.5px;color:#64748B;font-family:'Inter',sans-serif;font-weight:500">
      @if($participants->total() > 0)
        <span style="color:#1E293B;font-weight:700">{{ $participants->firstItem() }}–{{ $participants->lastItem() }}</span>
        <span style="color:#94A3B8"> / {{ $participants->total() }}名</span>
      @else
      ...
      <tbody>
        ...
        <tr>
          <td style="padding-left:20px;width:44px">
            <span style="font-size:11px;font-weight:600;color:#CBD5E1;font-family:'Inter',sans-serif">{{ str_pad($loop->index + ($participants->currentPage()-1)*$participants->perPage() + 1, 2, '0', STR_PAD_LEFT) }}</span>
          </td>
          <td>
            <div style="display:flex;align-items:center;gap:12px">
              <div style="width:38px;height:38px;border-radius:11px;background:{{ $avc }};display:flex;align-items:center;justify-content:center;color:#fff;font-size:15px;font-weight:800;flex-shrink:0;font-family:'Inter',sans-serif;box-shadow:0 0 0 2.5px #fff,0 0 0 4.5px {{ $avc }}50;letter-spacing:-0.01em">{{ $ini }}</div>
              <div>
                <p style="font-size:13.5px;font-weight:700;color:#1E293B;line-height:1.3;font-family:'Noto Sans JP',sans-serif;white-space:nowrap">{{ $p->company?->company_name ?? '（会社名未登録）' }}</p>
                <p style="font-size:11.5px;color:#94A3B8;line-height:1.4;font-family:'Noto Sans JP',sans-serif;white-space:nowrap;margin-top:1px">{{ $p->name ?? '（氏名未登録）' }}</p>
              </div>
            </div>
          </td>
          <td>
            <p style="font-size:13px;color:#475569;font-family:'Noto Sans JP',sans-serif;white-space:nowrap;line-height:1.3;font-weight:500">{{ $p->role_title ?? '—' }}</p>
            @if($p->company?->industry?->name)
              <span style="display:inline-block;margin-top:4px;background:#F1F5F9;color:#64748B;font-size:11px;font-weight:600;padding:2px 8px;border-radius:5px;font-family:'Noto Sans JP',sans-serif;white-space:nowrap">{{ $p->company->industry->name }}</span>
            @else
              <p style="font-size:11.5px;color:#CBD5E1;margin-top:2px">—</p>
            @endif
          </td>
          <td>
            @if($p->company?->prefecture?->name)
          ...
          <td>
            <p style="font-family:'Inter',sans-serif;font-size:12.5px;color:#475569;font-weight:600;white-space:nowrap">{{ $p->created_at->format('Y/m/d') }}</p>
            <p style="font-family:'Inter',sans-serif;font-size:11px;color:#94A3B8;white-space:nowrap;margin-top:1px">{{ $p->created_at->format('H:i') }}</p>
          </td>
...
@if($participants->hasPages())
  <div style="margin-top:16px">{{ $participants->links() }}</div>
@endif
